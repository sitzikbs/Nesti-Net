...
def load_shape(point_filename, normals_filename, curv_filename, pidx_filename, noise_level=None):
    ...
    return Shape(pts=pts, kdtree=kdtree, normals=normals, curv=curvatures, pidx=patch_indices, noise_level=noise_level)
...
class Shape():
    def __init__(self, pts, kdtree, normals=None, curv=None, pidx=None, noise_level=None):
        self.pts = pts
        self.kdtree = kdtree
...
class PointcloudPatchDataset(data.Dataset):
    ...
    def __init__(self, root, shape_list_filename, patch_radius, points_per_patch, patch_features,
        ...
        # get all shape noise levels in the dataset
        self.noise_levels = []
        if os.path.exists(os.path.join(root, self.shape_list_filename[:-4] + '_noise_levels.txt')):
            with open(os.path.join(root, self.shape_list_filename[:-4] + '_noise_levels.txt')) as f:
                self.noise_levels = f.readlines()
            self.noise_levels = [x.strip() for x in self.noise_levels]
            self.noise_levels = list(filter(None, self.noise_levels))
            self.noise_levels = [float(x) for x in self.noise_levels]
        else:
            print(' no noise levels file')
            self.noise_levels = np.zeros_like(self.shape_names)
    ...
    def __getitem__(self, index):
        ...
        return (patch_pts,) + patch_feats + (trans,) + (effective_points_num,)
    ...
    def load_shape_by_index(self, shape_ind):
        ...
        return load_shape(point_filename, normals_filename, curv_filename, pidx_filename, noise_level=self.noise_levels[shape_ind])
