...
# Note - to train the multi-scale switching model there must be a noise level file.

#Execute
# python train_n_est_w_switching.py   --model='ms_sw_n_est' --gpu=0 --log_dir='my_ms_sw' --patch_radius 0.01 0.05   --loss_type='sin' --batch_size=64 --num_point=512 --identical_epochs=0 --num_gaussians=8 --gmm_variance=0.0156 --learning_rate=0.0001 --max_epoch=1000 --momentum=0.9 --optimizer='adam'  --weight_decay=0.0 --decay_rate=0.7 --decay_step=491520 --trainset='trainingset_whitenoise.txt' --testset='validationset.txt' --desc='Testing normal estimation with integrated learned scale switching, sin loss'

parser = argparse.ArgumentParser()
#Parameters for learning
parser.add_argument('--gpu', type=int, default=0, help='GPU to use [default: GPU 0]')
parser.add_argument('--desc', type=str, default='My training run with switching', help='description')
parser.add_argument('--data_path', type=str, default='data/pcpnet/', help='Relative path to data directory')
parser.add_argument('--model', default='ms_sw_n_est', help='Model name [default: 3dmfv_net_cls]')
parser.add_argument('--log_dir', default='my_ms_sw', help='Log dir [default: log]')
...
parser.add_argument('--identical_epochs', type=int, default=True,
                    help='use same patches in each epoch, mainly for debugging')
parser.add_argument('--loss_type', type=str, default='cos', help='loss type [euclidean]')

parser.add_argument('--outputs', type=str, nargs='+', default=['unoriented_normals', 'noise'],
...
parser.add_argument('--patch_radius', type=float, default=[0.01, 0.05], nargs='+', help='patch radius'
                            ' in multiples of the shape\'s bounding box diagonal, currently supports only teo')
...
# Parameters for GMM
parser.add_argument('--num_gaussians', type=int, default=8, help='number of gaussians for gmm, [default: 8, i.e. 512 '
                                                                 'gaussians, for other values you should adjust the'
                                                                 ' architecture]')
parser.add_argument('--gmm_variance', type=float,  default=0.0156, help='variance for grid gmm, recommended use '
                                                                        '(1/num_gaussians)^2')
...
os.system('cp train_n_est_w_switching.py %s' % (LOG_DIR)) # bkp of train procedure
...
def train(gmm):
    ...
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            points_pl, noise_gt_pl, normal_pl, w_pl, mu_pl, sigma_pl, n_effective_points = MODEL.placeholder_inputs(BATCH_SIZE, NUM_POINT, gmm, PATCH_RADIUS)
            ...
            # Get model and loss
            noise_pred, n_pred, fv = MODEL.get_model(points_pl, w_pl, mu_pl, sigma_pl, is_training_pl, PATCH_RADIUS, original_n_points=n_effective_points, bn_decay=bn_decay, weight_decay=WEIGHT_DECAY)
            loss, cos_ang = MODEL.get_loss(noise_pred, noise_gt_pl, n_pred, normal_pl, loss_type=LOSS_TYPE)
...
def eval_one_epoch(sess, ops, gmm, test_writer, testset_loader, dataset):
    ...
    for batch_idx, data in test_enum:
        ...
        summary, step, loss_val, noise_est, n_est, cos_ang = sess.run([ops['merged'], ops['step'],
                                                      ops['loss'], ops['noise_pred'],
